...
// app.use("/api",require("./routes/Proiect"));
// app.use("/api",require("./routes/MembruEchipa"));

//middleware pentru eroare 500
app.use((err,req,res,next)=>{
    res.status(500).json({err: "Nu a functionat."})
})
...
app.post("/proiecte/:proiectId/membruEchipa", async (req, res, next) => {
  try {
    ...
    if (proiect) {
      ...
      res.status(201).json({ message: 'Membrul echipei a fost adaugat!'});
    } else {
 ...
 */
  ...
  app.get("/proiecte/:proiectId/membriiEchipa", async (req, res, next) => {
    try {
      ...
      if (proiect) {
        res.status(200).json(proiect.membriiEchipa);
      } else {
        res.status(404).json({ message: '404 - Proiectul nu a fost gasit!'});
      }
    } catch(err) {
 ...
 */
 ...
 app.get("/proiecte/:proiectId/bugs", async (req, res, next) => {
  try {
    ...
    if (proiect) {
      res.status(200).json(proiect.bugs);
    } else {
      res.status(404).json({ message: '404 - Proiectul nu a fost gasit!'});
    }
  } catch(err) {
 ...
 */
 ...
 /**
 * PUT pentru a actualiza un proiect
 */

 app.put("/proiecte/:proiectId", async (req, res, next) => {
  try {
    const proiect = await Proiect.findByPk(req.params.proiectId);
    if (proiect) {
      proiect.numeProiect = req.body.numeProiect
      proiect.numeEchipa = req.body.numeEchipa
      await proiect.save();
      res.status(202).json({ message: 'Proiectul a fost actualizat cu succes!' });
    } else {
      res.status(404).json({ message: '404 - Proiectul nu a fost gasit!'});
    }
  } catch (err) {
